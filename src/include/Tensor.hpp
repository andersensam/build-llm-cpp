...
/**
 * Enum class used for searching through a Matrix
 */
...
/* Private functions*/
     ...
     * @returns Returns an initializer list with rewritten coordinates
     */
    std::array<size_t, 2> rewrite(const std::initializer_list<size_t>& c) const {
        // Handle the case where our base Tensor has rank == 2, but where we may only
        // be returning a vector
        // NOLINTBEGIN(cppcoreguidelines-pro-bounds-pointer-arithmetic)
        if (c_num_slice_dims == 1) {
            // If we are slicing across dim 0, translate that part of the coordinate and then
            // use the raw second coordinate from m_other_axes
            if (c_slice_dims.at(0) == 0) {
                return {m_dim0_map.at(c.begin()[0]), m_other_axes.at(1)};
            }
            return {m_other_axes.at(0), m_dim0_map.at(c.begin()[0])};
        }
        // If we have a real 2-D mapping, fetch m_dim1_map too
        if (!m_dim1_map.empty()) {
            return {m_dim0_map.at(c.begin()[0]), m_dim1_map.at(c.begin()[1])};
        }
        // Otherwise pass through the second axis
        return {m_dim0_map.at(c.begin()[0]), c.begin()[1]};
     ...
     */
    const std::vector<size_t>& highrank_rewrite(const std::initializer_list<size_t>& c) {
...
public:
    ...
    TensorSlice(const Tensor<T>& tensor, const std::initializer_list<size_t>& axes, const Tensor<size_t>& idxs,
        ...
        // NOLINTBEGIN(cppcoreguidelines-pro-bounds-pointer-arithmetic)
        // We only have 2 cases: we are either creating a row / column vector (1-D Tensor), or
        // we are creating a smaller Matrix from a larger Matrix or Tensor
        if (axes.size() == 1) {
            // Store the axis we are slicing across
            c_slice_dims.at(0) = axes.begin()[0];
            // Given the result in a 1-D Tensor, col / row doesn't matter, so just use row
            c_rows = idxs.elements();
            // Iterate over the values in idxs, assigning new indices
    ...
    const T& at(const std::initializer_list<size_t>& c) {
        ...
        if (tm.rank() == 2) {
            std::array<size_t, 2> rw = rewrite(c);
            return tm.at({rw.at(0), rw.at(1)});
        }
        return tm.at(highrank_rewrite(c));
    }
